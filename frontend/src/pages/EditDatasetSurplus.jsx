...
const EditDatasetSurplus = () => {
  ...
  useEffect(() => {
    ...
    if (!token) { navigate("/"); }
    else { getKriteriaSurplus(); getDatasetById(); }
  }, [navigate, id]);

  const getKriteriaSurplus = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`${API_URL}/kriteria-surplus`,
        { headers: { Authorization: `Bearer ${token}` } });
      setKriteriaSurplus(res.data);
  ...
  const getDatasetById = async () => {
    try {
      setLoadingData(true);
      const token = localStorage.getItem("token");
      const res = await axios.get(`${API_URL}/dataset-surplus/${id}`,
        { headers: { Authorization: `Bearer ${token}` } });
  ...
  const updateDataset = async (e) => {
    ...
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      await axios.patch(`${API_URL}/dataset-surplus/${id}`, { nilai },
  ...
  if (kriteriaSurplus.length === 0) return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
          ...
          <div className="mb-8">
            <div className="backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl rounded-2xl p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-2xl font-bold text-white">Edit Dataset Surplus Kalori</h1>
              </div>
              <p className="text-white/70 ml-12">Perbarui data pasien surplus kalori</p>
            </div>
          </div>
...
export default EditDatasetSurplus;
