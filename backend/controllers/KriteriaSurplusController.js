...
const getKriteriaSurplusById = async (req, res) => {
    try {
        const kriteria = await KriteriaSurplus.findOne({
...
const createKriteriaSurplus = async (req, res) => {
    ...
    try {
        ...
        const newKriteria = await KriteriaSurplus.create({
...
const updateKriteriaSurplus = async (req, res) => {
    try {
        const kriteria = await KriteriaSurplus.findOne({ where: { id: req.params.id } });
        if (!kriteria) return res.status(404).json({ msg: "Kriteria tidak ditemukan" });
